...
Each session entry holds:
  ...
  - overlap_pcm: the tail of the previous window's audio (raw PCM bytes) that
    will be prepended to the next window's buffer before dispatch. Provides
    Whisper with acoustic context across window boundaries without relying on
    text-based initial_prompt injection. Reset to b"" between clips.
  - overlap_session_start: the session-level time (seconds from clip start) at
    which overlap_pcm begins. Used to convert pool-returned word timestamps
    (relative to overlap_pcm's t=0) into session-level times. Reset to 0.0
    between clips.
  - emitted_until: the session-level end time (seconds) of the last word that
    has already been sent to the App container. Words in a new window whose
    session-level end time is <= emitted_until are in the overlap region and
    are silently skipped. Reset to 0.0 between clips.
...
# Used by TranscriptionService to compute the byte offset of the overlap
# region from Whisper's word-boundary timestamps.
...
class SessionEntry:
    ...
    # Tail of the previous window's audio, prepended to the next buffer
    # before Whisper dispatch to provide acoustic context across boundaries.
    overlap_pcm:           bytes      = b""
    # Session-level time (seconds from clip start) at which overlap_pcm begins.
    # All pool-returned word timestamps are offset by this value to produce
    # session-level times.
    overlap_session_start: float      = 0.0
    # Session-level end time of the last word already emitted to the App
    # container. Words whose session-level end <= this value are in the
    # overlap region and must not be re-emitted.
    emitted_until:         float      = 0.0
